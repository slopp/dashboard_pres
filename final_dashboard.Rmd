...
renderLeaflet({
  data <- getKnownAreas()
  data <- data %>% separate(weather_underground, 
                         into = c("lat","lon"),
                         sep=",")
...
renderPlotly({
  ...
  p <- forecast() %>% 
    ggplot(aes(x = hour, y = forecast, color = resorts)) +
    geom_jitter() +
    ylab("Snowfall (in)") +
    xlab("Hour Forecast") +
    theme_minimal() +
  ...
  ggplotly(p,tooltip = c("forecast", "resorts"))
})
```
...
renderPlot({
  ...
  ggplot(all_weather, aes(x=type, y=temp, label=resort, color=temp))+
    geom_label_repel() +
...
renderPlot({
  ...
  min_resort <- which(d$base == min(d$base))
