...
getCurrentWeather <- function(area_name){
  ...
  base_url <- "http://api.wunderground.com/api/####API_KEY####/conditions/q/"
...
For this one I'm thinking of just doing the helloworld example
...
Now it is time to show the actual ski resort dashboard


---
template: flexdashboard

### Parameterized flexdashboards are awesome!
### They can also be interactive (with htmlwidgets)
...
### Problem! 
### Could Call API >> Data Changes
### OR Data Change >> Call API
...
-  In our dashboard: 
```{r}
areas <- reactiveFileReader(n, "areasRdat", readRDS)
conditions <- reactive({
  areas[[input$resort]]
})
```
